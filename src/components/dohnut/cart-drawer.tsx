  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetContent
        ...
        // RX-03: bottom-sheet on mobile (where the gesture is natural),
        // side-panel on desktop where the cart should not block the product
        // grid. Caps at sm:max-w-md (448px) to match the Concierge panel.
        className="mx-auto flex max-h-[82vh] w-full max-w-lg flex-col gap-0 overflow-hidden rounded-t-[32px] border-x border-t border-[var(--color-dowgnut-blue-dark)]/15 bg-[var(--color-dowgnut-cream)]/95 p-0 shadow-2xl backdrop-blur-2xl md:right-4 md:left-auto md:mx-0 md:max-h-[88vh] md:w-[420px] md:rounded-3xl md:border"
      >
              ...
              <ul className="flex flex-col gap-2">
                {cart.map((item) => (
                  <motion.li
                    ...
                    className="group relative flex items-center gap-3 rounded-2xl border border-[var(--color-dowgnut-blue-dark)]/8 bg-white p-2.5 shadow-xs transition-shadow duration-150 hover:shadow-sm"
                  >
                    {/* Donut image preview */}
                      ...
                      <div className="mt-1.5 flex items-center justify-between">
                        ...
                        <div className="inline-flex items-center rounded-full border border-[var(--color-dowgnut-blue-dark)]/15 bg-white shadow-xs">
                          <button
                            ...
                            aria-label={`Decrease quantity for ${item.donut.name}`}
                            disabled={item.quantity <= 1}
                            className="relative inline-flex size-7 items-center justify-center rounded-l-full text-[var(--color-dowgnut-blue-dark)] transition-colors hover:bg-black/5 disabled:opacity-30 cursor-pointer active:scale-95"
                          >
                          ...
                          <span
                            ...
                            className="min-w-6 text-center text-xs font-black tabular-nums text-[var(--color-dowgnut-blue-dark)] select-none"
                          >
                            {item.quantity}
                          </span>
                          <button
